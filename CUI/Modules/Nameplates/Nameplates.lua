...
--------------------------------------------------------
	...
	function Module:LoadConfig()
		...
		C_NamePlate.SetNamePlateSize(Config.clickableWidth, Config.clickableHeight)
...
--------------------------------------------------------
	...
	local function Update_Health(self, event, ...)
		...
		local HealthPct = UnitHealthPercent(self.owner.unit, true, CurveConstants.ScaleTo100)
		...
		--else
			self.Font:SetText(format("%.2f %%", HealthPct))
		--end
	end
	...
	function Module:UpdatePlate(Unitframe)
		...
		if UnitNameplateShowsWidgetsOnly(Unitframe.unit) then return end
	...
	function Module:UpdateGeneral(Unit)
...
function Module:Init()
	...
	self:SetScript("OnEvent", Module.OnEvent)
	self:LoadConfig()
end

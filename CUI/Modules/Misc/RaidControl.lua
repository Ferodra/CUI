...
local Module = CreateFrame("Frame", "RaidControlFrame", E.Parent)
Module.Autoload = true
--------------------------------------------------------------------------------------------

local _
local C_PartyInfo_DoCountdown = C_PartyInfo and C_PartyInfo.DoCountdown

local Buttons = {{"Readycheck", "Readycheck", "PerformReadyCheck"}, {"PullTimer", "Pull Timer", "PerformPullTimer"}, {"StopPull", "Stop Pull", "StopPull"}}

function Module:PerformReadyCheck()
	DoReadyCheck()
end

function Module:StopPull()
	local InGroup, InRaid = IsInGroup(), IsInRaid()
	Module:SendPullTimer(0, InGroup, InRaid)
end

function Module:PerformPullTimer()
	local Time = Module:GetPullTime()
	if Time ~= "" then
		Time = tonumber(Time)
		if Time > 0 then
			local InGroup, InRaid = IsInGroup(), IsInRaid()
			...
			if InGroup or InRaid then
				Module:SendPullTimer(Time, InGroup, InRaid)
			else
				if Time < 15 then
					if Time <= 2 then
						E:print(("Whoa there! Let's hope your group can react within %d seconds!"):format(Time))
					else
...
function Module:SendPullTimer(Time, InGroup, InRaid)
	...
	if not E.IsRetail then
		...
		Module.Buttons["PullTimer"].EditBox:ClearFocus()
...
function Module:StyleButton(Button, Width, Height, GenerateFont, StripTextures, OverrideNormal, OverrideHighlight, OverridePushed)
	...
	if OverrideNormal then
		Button:SetNormalTexture([[Interface\AddOns\CUI\Textures\borders\WHITE8X8]])
		Button:GetNormalTexture():SetVertexColor(0.15, 0.15, 0.15, 0.6)
	end
	
	if OverrideHighlight then
		Button:SetHighlightTexture([[Interface\AddOns\CUI\Textures\borders\WHITE8X8]])
		Button:GetHighlightTexture():SetVertexColor(0.16, 0.16, 0.16, 0.65)
	end
	
	if OverridePushed then
		Button:SetPushedTexture([[Interface\AddOns\CUI\Textures\borders\WHITE8X8]])
		Button:GetPushedTexture():SetVertexColor(0.07, 0.07, 0.07, 0.75)
	end
end

function Module:AddClosedButton()
...
function Module:AddPanelCloseButton()
...
function Module:AddButtons()	
	...
	for _, data in pairs(Buttons) do
		...
		self.Buttons[data[1]]:SetScript("OnClick", Module[data[3]])
	...
	local MarkerBtn = CompactRaidFrameManager.displayFrame.raidMarkers
	--local Buttons = MarkerBtn["raidMarker"..i]
	...
	-- Reposition Pull Button
		...
		self.Buttons["PullTimer"].EditBox:SetScript("OnChar", function(self, input)
			...
			-- Limit to 60 within DBM and BigWigs
			if E.IsRetail then
		...
		self.Buttons["PullTimer"].EditBox:SetScript("OnKeyDown", function(self, key)
			if key == "ENTER" then
				...
				if Module.db.pullOnEnter then
					Module:PerformPullTimer()
				end
			end
		end)
...
function Module:GetPullTime()
	return self.Buttons["PullTimer"].EditBox:GetText()
end

function Module:OnEvent(event, ...)
	if (IsInGroup() or IsInRaid()) and (UnitIsGroupLeader('player') or UnitIsGroupAssistant('player')) then
		self:Show()
	else
		self:Hide()
	end
...
function Module:Construct()
	...
	E:CreateMover(self, "RaidControl", nil, nil, nil, "A panel that holds various functionality to administrate a raid.", "misc")
...
-- CUI:LoadModule("RaidControl"):Hide()
E:AddModule("RaidControl", Module)
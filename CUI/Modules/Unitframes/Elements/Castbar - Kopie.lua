...
local CO, BC, UF, TT = E:LoadModules("Config", "Castbar", "Unitframes", "Tooltip")
...
function BC:LoadSingleBar(Frame, GlobalConfig, Config)
	-- Automatically creates the new bar when it doesn't exist
	if not Frame.Castbar then
		self:Create(Frame, true)
	end
	local Bar = Frame.Castbar
	if not Bar then return end
	
	GlobalConfig, Config = GlobalConfig or BC.db.unitframe.units.all.castbar, Config or BC.db.unitframe.units[Frame.ConfigKey].castbar
...
function BC:LoadConfig()
	local GlobalConfig, Config = BC.db.unitframe.units.all.castbar
	...
	for _, Bar in pairs(self.Bars) do
		self:LoadSingleBar(Bar.Owner, GlobalConfig, BC.db.unitframe.units[Bar.Owner.ConfigKey].castbar)
	end
end

function BC:Toggle(Unitframe)
...
function BC:ToggleMovers(state)
...
local function Castingbar_FinishCast(self, state, holdTime)
	...
	local Color = (state == true or state == nil) and BC.DBColors.Success or BC.DBColors.Failed
...
local function Bar_OnEvent(self, event, ...)
	...
	if event == "UNIT_SPELLCAST_START" or ((event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_FOCUS_CHANGED") and UnitCastingInfo(self.Owner.unit)) then
		...
		if self.SpellName then
			...
			if self.Owner.unit ~= "player" and self.notInterruptible then BC:UpdateLagBar(self, false) else BC:UpdateLagBar(self, true) end
...
function BC:UpdateBarUnit(Bar, Unit)
	Bar.unit = Unit
	BC:UpdateEvents(Bar)
end
...
function BC:Create(Frame, doNotLoad)
...
function BC:Init()
	...
	local BarUnits = {'player','target','targettarget','focus','focustarget','pet','boss1','boss2','boss3','boss4','boss5', 'arena1','arena2','arena3','arena4','arena5'}
	
	for k,v in pairs(BarUnits) do
		self:Create(UF:GetUnitframe(v))
	end

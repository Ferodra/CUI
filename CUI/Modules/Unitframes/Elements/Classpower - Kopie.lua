...
Module.UpdateFuncs = {
	-- DeathKnight Runes
	-- those are updated most frequently
	[5] = function(self, data)
	...
	-- Soulshard Fragments
	[7] = function(self, data)
		if self.PlayerSpec == 3 then
...
function Module:LoadConfig()
	...
	E:UpdateMoverDimensions(self.Holder, self.db.width, self.db.height)
...
function Module:UpdateValue(data)
	if not self.CurrentPower or not self.PowerId or not self.CurrentPower.Bars then return end
	
	-- For more modularity
	if self.UpdateFuncs[self.PowerId] then
		self.UpdateFuncs[self.PowerId](self, data)
	else
		self.UpdateFuncs[999](self, data)
	end
end
...
function Module:__OnEvent(event, unit, ...)

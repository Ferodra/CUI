...
local function GetOptionsTable_ClassColor(class, order)
	local color = {
	  name = E.ClassNames[class] or "Unknown",
	  type = "color",
	  desc = string.format("%s '%s'", L["Modifies the color of the class"], E.ClassNames[class] or "Unknown"),
	  order = order,
	  get = function(info)
			local c = CO.db.profile.colors.classes[class]
			if not c then
				c = E.ClassColors[E.ClassIDByName[class]]
			end
			return c[1], c[2], c[3], c[4]
	  end,
	  set = function(info, r, g, b, a)
			local c = E:GetClassColorByClassName(class)
			
			c[1], c[2], c[3], c[4] = r, g, b, a
			
			E:LoadModule("Unitframes"):UpdateAllUF()
	  end,
	}
...
local function GetOptionsTable_LayoutBars(title, type, index, module)
	local color = {
	  name = title,
	  type = "color",
	  hasAlpha = true,
...
local function GetOptionsTable_ZoneColor(type, order)
	local config = {
		...
		set = function(info, r, g, b)
			...
			E:LoadModule("Layout"):UpdateLocationZone()
		end,
	}
...
local function AuraEntry_Add(info, value)
	...
	if not ID then
		local Name = value
		ID = select(7, E:GetSpellInfo(Name))
	end
...
local UnitEntry_Selected = nil

local function UnitEntry_PushUpdate()
	-- Update UFs and Nameplates
	NP:LoadConfig()
	UF:UpdateAllUF()
end

local function UnitEntry_Add(info, Name)
	if (Name and (Name == '' or Name:gsub("%s+", "") == '')) or not Name then return end
	
	
	if not CO.db.global.colors.units[Name] then
		CO.db.global.colors.units[Name] = {enabled = true, color = {["r"] = 0.5, ["g"] = 0.5, ["b"] = 0.5}}
	
		UnitEntry_PushUpdate()
	else
		E:print("Unit " .. Name .. " already exists!")
	end
	
	UnitEntry_Selected = Name
end

local function UnitEntry_Delete()
	if not UnitEntry_Selected then return end
	
	if CO.db.global.colors.units[UnitEntry_Selected] then
		CO.db.global.colors.units[UnitEntry_Selected] = nil
		UnitEntry_Selected = nil
	...
	args = {
		...
		classGroup = {
			...
			args = {
				...
				EVOKER = GetOptionsTable_ClassColor("EVOKER", 34),
				...
				reset = {
					...
					name = L["Reset"],
					order = 1001,
					func = function() for k,v in pairs(E.ConfigDefaults.profile.colors.classes) do for key, color in pairs(v) do CO.db.profile.colors.classes[k][key] = color end end; E:LoadModule("Unitframes"):UpdateAllUF(); end,
				},
			},
		},
		powerGroup = {
			...
			args = {
				...
				Icicles 		=  GetOptionsTable_PowerColor(33, 140),
				AlternatePower 	=  GetOptionsTable_PowerColor(10, 200),
				...
				reset = {
					...
					name = L["Reset"],
					order = 1001,
					func = function() for k,v in pairs(E.ConfigDefaults.profile.colors.powers) do for key, color in pairs(v) do CO.db.profile.colors.powers[k][key] = color end end; E:LoadModule("Unitframes"):UpdateAllUF(); end,
				},
			},
		},
		reactionGroup = {
			...
			args = {
				...
				reset = {
					...
					name = L["Reset"],
					order = 1001,
					func = function() for k,v in pairs(E.ConfigDefaults.profile.colors.reactions) do for key, color in pairs(v) do CO.db.profile.colors.reactions[k][key] = color end end; E:LoadModule("Unitframes"):UpdateAllUF() end,
				},
			},
		},
		readycheckGroup = {
		...
		layoutBarsGroup = {
			...
			args = {
				...
				xpUseClassColor = {
					...
					set = function(info, value) CO.db.profile.colors.layoutBars.barExperienceNormal.useClassColor = value; E:LoadModule("Bar_Experience"):LoadConfig(); end,
				},
				xpNormal 			= GetOptionsTable_LayoutBars(L["XPBarNormal"], "barExperienceNormal", 100, E:LoadModule("Bar_Experience")),
				xpRested 			= GetOptionsTable_LayoutBars(L["XPBarRested"], "barExperienceRested", 200, E:LoadModule("Bar_Experience")),
				...
				azUseClassColor = {
					...
					set = function(info, value) CO.db.profile.colors.layoutBars.barAzerite.useClassColor = value; E:LoadModule("Bar_Azerite"):LoadConfig(); end,
				},
				azOverlay 			= GetOptionsTable_LayoutBars(L["AzeriteBarOverlay"], "barAzerite", 350, E:LoadModule("Bar_Azerite")),
				newLine = {type = "description", name = "", order = 350},
				repHeader = {
					type = "header",
					name = L["ReputationBar"],
					order = 500,
				},
				repUseClassColor = {
					type = "toggle",
					...
					name = L["UseClassColor"],
					desc = L["UseClassColorDesc"],
					get = function() return CO.db.profile.colors.layoutBars.barReputation.useClassColor end,
					set = function(info, value) CO.db.profile.colors.layoutBars.barReputation.useClassColor = value; E:LoadModule("Bar_Reputation"):LoadConfig(); end,
				},
				repOverlay 			= GetOptionsTable_LayoutBars(L["ReputationBarOverlay"], "barReputation", 520, E:LoadModule("Bar_Reputation")),
				newLine = {type = "description", name = "", order = 2000},
				reset = {
					type = "execute",
					name = L["Reset"],
					order = 2001,
					func = function() for k,v in pairs(E.ConfigDefaults.profile.colors.layoutBars) do for key, color in pairs(v) do CO.db.profile.colors.layoutBars[k][key] = color end end; E:LoadModule("Bar_Experience"):LoadConfig(); E:LoadModule("Bar_Azerite"):LoadConfig(); E:LoadModule("Bar_Reputation"):LoadConfig(); end,
				},
			},
		},
		zoneGroup = {
		...
		aurasGroup = {
			...
			args = {
				...
				desc = {
					type = "description",
					order = 1,
					name = "Here, you can define specific colors for auras",
					fontSize = "small",
				},
				...
				multiResult = {
					...
					hidden = function() return not HasMultiResult or CD:GetSpellMatchNumber(CurrentAuraToAdd) < 1 end,
				},
					...
					values = function()
						...
						for k, v in pairs(CO.db.profile.colors.auras) do
							if E:GetSpellInfo(k) then
								lookupTable[k] = string.format("%s (%s)", select(1, E:GetSpellInfo(k)), k)
							end
						end
		...
		unitGroup = {
			...
			args = {
				desc = {
					type = "description",
					order = 1,
					name = "Here, you can define colors for specific unit names",
					fontSize = "small",
				},
				newLine = {type="description", name="", order=5},
				add = {
				...
				selection = {
					...
					values = function()
						...
						for k, v in pairs(CO.db.global.colors.units) do
							lookupTable[k] = k
						end
						
						return lookupTable
					end,
					get = function() return UnitEntry_Selected end,
					set = function(info, value) UnitEntry_Selected = value end,
				},
				delete = {
					type = "execute",
					name = L["Delete"],
					order = 12,
					hidden = function() return not UnitEntry_Selected end,
					func = UnitEntry_Delete,
				},
				...
				unitEnable = {
					...
					order = 21,
					hidden = function() return not UnitEntry_Selected end,
					get = function() return CO.db.global.colors.units[UnitEntry_Selected].enabled end,
					set = function(info, value) CO.db.global.colors.units[UnitEntry_Selected].enabled = value; 
						UnitEntry_PushUpdate()
					end,
				},
				...
				unitColor = {
					...
					order = 26,
					get = function(info)
						if not UnitEntry_Selected or not CO.db.global.colors.units[UnitEntry_Selected] then return end
						local c = CO.db.global.colors.units[UnitEntry_Selected].color
						return c.r, c.g, c.b
					end,
					set = function(info, r, g, b)
						local c = CO.db.global.colors.units[UnitEntry_Selected].color
					...
					disabled = function() return not CO.db.global.colors.units[UnitEntry_Selected].enabled end,
					hidden = function() return not UnitEntry_Selected end,
				},
			},
		},

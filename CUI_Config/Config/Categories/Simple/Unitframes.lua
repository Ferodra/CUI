...
--------------------------------------------------------------------------
local OverrideFont_EnableHeight, OverrideFont_EnableType, OverrideFont_EnableFlags
local OverrideFont_Types = {'fontHeight', 'fontType', 'fontFlags'}
local OverrideFont_Height, OverrideFont_Type, OverrideFont_Flags
...
local function ApplyGlobalFontOverride()
	...
	for _, OverrideType in pairs(OverrideFont_Types) do
		if OverrideType == 'fontHeight' then
			OverrideValue = OverrideFont_Height
		elseif OverrideType == 'fontType' then
			OverrideValue = OverrideFont_Type
		elseif OverrideType == 'fontFlags' then
...
local function SetUnitDummys(state)
	...
	for k, v in pairs(self.Frames) do
		if state == true then
			...
			v.Fonts.Health.unit = "player"
			v.Fonts.Power.unit = "player"
		else
			...
			v.Fonts.Health.unit = v.BackupUnit
			v.Fonts.Power.unit = v.BackupUnit
		end
...
local function GetOptionsTable_HealthBar(groupName)
	...
	local config = {
		...
		set = function(info, value) CO.db.profile.unitframe.units[groupName].health[ info[#info] ] = value; UF.Modules["Health"]:LoadConfig(); UF:LoadProfileForUnits(groupName); UF.Modules["HealPrediction"]:LoadConfig(); UF:LoadHolderConfig(groupName) end,
		args = {
			position = {
				type = "execute",
				order = 1,
...
local function GetOptionsTable_CastBar(groupName)
	...
	local config = {
		...
		set = function(info, value) CO.db.profile.unitframe.units[groupName].castbar[ info[#info] ] = value; E:LoadModule("Castbar"):LoadConfig() end,
		args = {
			enable = {
				type = "toggle",
				order = 1,
			...
			show = {
				...
				name = L["Toggle"],
				func = function()
					UF:PerformForUnits(groupName, E:LoadModule("Castbar").Toggle)
				end,
			},
			barSettings = {
				type = "group",
...
local function GetOptionsTable_Restingindicator()
	local config = {
		...
		set = function(info, value) CO.db.profile.unitframe.units.player.restingIndicator[ info[#info] ]  = value; UF:LoadProfileForUnits("player"); end,
		args = {
			enable = {
				type = "toggle",
				order = 1,
			...
			position = {
				...
				name = L["Position"],
				values = E.Positions,
				disabled = function() return not CO.db.profile.unitframe.units.player.restingIndicator.enable end,
			},
			offsetX = {
				order = 3,
				type = 'range',
						...
						values = function(self)
							...
							for ClassID = 1, #IconSet do
								for SpecID = 1, #IconSet[ClassID] do
									Values[IconSet[ClassID][SpecID].SpecID] = '|T' .. (IconSet[ClassID][SpecID].IconID or 'error') .. ':0|t ' .. E:GetClassColorizedText(IconSet[ClassID][SpecID].SpecName, ClassID)
			...
			fontGroup = {
				...
				args = {
					enableHeight = {
						...
						func = function() OverrideFont_EnableHeight = true end,
					...
					enableType = {
						...
						desc = 'Enables the global font type override. Use with caution!',
						func = function() OverrideFont_EnableType = true end,
					...
					enableFlags = {
						...
						func = function() OverrideFont_EnableFlags = true end,
					...
					apply = {
						...
						disabled = function() return not (OverrideFont_EnableFlags or OverrideFont_EnableType or OverrideFont_EnableHeight) end,
					},
				}
			},
		},
